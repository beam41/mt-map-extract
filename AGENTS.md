...
| `wiki/` | the DokuWiki generator (`wiki`): `Program.cs`, `Data.cs` (pak gathering), `RenderVehicles.cs` / `RenderParts.cs` / `RenderCargos.cs` (page templates), `Format.cs` (display rules), `WikiOptions.cs` (its own CLI). Writes `out/wiki/` (vehicles/, parts/, cargos/, cargo_space/, list_of_*.txt, vehicle_comparison.txt) — no json. |
